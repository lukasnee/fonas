...
struct Hex {
    ...
    static const char *format(std::array<char, N> &out_buff,
                              const uint8_t *in_data, size_t in_size,
                              const char *delimiter = "",
                              const char *byte_fmt = "%02X") {
        return format(out_buff.data(), out_buff.size(), in_data, in_size,
                      delimiter, byte_fmt);
    }

    static const char *format(char *out_buff_data, size_t out_buff_size,
                              const uint8_t *in_data, size_t in_size,
                              const char *delimiter = "",
                              const char *byte_fmt = "%02X") {
        if (!in_data) {
        ...
        for (; i < in_size && cp < out_buff_size - 1; i++) {
            cp += std::snprintf(out_buff_data + cp, out_buff_size - cp,
                                byte_fmt, in_data[i]);
            if (i + 1 < in_size) {
                cp += std::snprintf(out_buff_data + cp, out_buff_size - cp,
                                    "%s", delimiter);
