...
/**
 * @brief Stream Buffer. Stream buffers allow a stream of bytes to be passed from an interrupt service routine to a task, or
 * from one task to another task. A byte stream can be of arbitrary length and does not necessarily have a beginning or end. Any
 * number of bytes can be written in one go, and any number of bytes can be read in one go. Data is passed by copy - the data is
 * copied into the buffer by the sender and out of the buffer by the read.
 *
...
public:
     ...
     *
     * @param buffer_size
     * @param trigger_level The number of bytes that must be in the stream buffer before a task that is blocked on the stream
     ...
     * @return std::size_t
     */
    std::size_t receive_from_isr(std::uint8_t *data, std::size_t size, BaseType_t *const higher_priority_task_woken = nullptr);
    ...
    /**
     * @brief Set the trigger level - the number of bytes that must be in the stream buffer before a task that is blocked on the
